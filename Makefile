...
CC     := gcc
BINARY := smnl
MODULES:= modules/sample.so modules/arp.so modules/ipv4.so modules/dns.so
...
modules/dns.so: modules/src/dns.c
	$(CC) $(CFLAGS) $(MFLAGS) $(DEFINES) $(INC) -Wno-format -o modules/dns.so modules/src/dns.c
